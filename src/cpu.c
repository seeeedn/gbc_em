...
//                 Other Helpers                 //
// ============================================= //
static inline void update_flags_zc(u8 result, u8 carry) {
    if (result == 0)
        SET_FLAG(cpu.F, FLAG_Z);
    if (carry)
        SET_FLAG(cpu.F, FLAG_C);
...
static inline void write_result(u8 *dest_reg, u8 value) {
    if (dest_reg == NULL) {
...
static u8 execute_cb_instruction(u8 cb_opcode) {
    ...
    switch (cb_opcode) {
        ...
        case 0x00 ... 0x07: {       // RLC r8
            ...
            update_flags_zc(mod_value, bit_7);
            write_result(dest_reg, mod_value);
        ...
        case 0x08 ... 0x0F: {       // RRC r8
            ...
            update_flags_zc(mod_value, bit_0);
            write_result(dest_reg, mod_value);
...
u8 execute_instruction(u8 opcode) {
    ...
    switch (opcode) {
        ...
        case 0xCB: {            // PREFIX CB
            ...
            cb_cycles = execute_cb_instruction(cb_opcode);
            //cpu.total_cycles += 4;
            break;
        }
        ...
        case 0x3C: {            // INC r8
            ...
            write_result(dest_reg, new_value);
...
u16 cpu_step() {
    ...
    // Sets IME after executing the next Instruction after the EI-Instruction
    if (cpu.ime_delay > 0) {
        cpu.ime_delay--;
        if (cpu.ime_delay == 0) {
            cpu.ime = true;
